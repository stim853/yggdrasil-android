...
class YggdrasilManager(
    ...
    /** Called when >2 peers are dead — should return new peer URIs to add.
     *  Note: adding peers at runtime is not supported by Yggdrasil mobile API.
     *  The returned list is logged for now; on next VPN restart fresh peers will be used. */
    private val onNeedPeerRefresh: (suspend () -> List<String>)? = null,
) {
    companion object {
    ...
    private suspend fun pollPeers(inst: Yggdrasil) {
        ...
        while (scope.isActive && ygg != null) {
            ...
            val dead  = peers.count { !it.up }

            if (count != lastCount) {
                AppLogger.i(TAG, "Peer count changed: $lastCount → $count (dead=$dead addr=$addr)")
                lastCount = count
            }
